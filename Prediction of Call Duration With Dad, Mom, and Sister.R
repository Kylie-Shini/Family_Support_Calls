...
combined_plot <- ggplot(path_filtered, aes(x = duration)) +
  ...
  geom_point(aes(y = mom), color = "purple", alpha = 0.6) +
  geom_point(aes(y = sister), color = "pink", alpha = 0.6) +
...
anim <- animate(combined_plot, nframes = 200, width = 600, height = 400,  # Reduced frames for faster speed
